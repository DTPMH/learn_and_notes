...
Status PathBoundsDecider::Process(
  ...
  PathBoundary fallback_path_boundary;
  for (size_t i = 0; i < fallback_path_bound.size(); ++i) {
    auto* fallback_boundary = fallback_path_boundary.add_boundary();
    fallback_boundary->set_left_l(std::get<2>(fallback_path_bound[i]));
    fallback_boundary->set_right_l(std::get<1>(fallback_path_bound[i]));
  }
  fallback_path_boundary.set_start_s(std::get<0>(fallback_path_bound[0]));
  fallback_path_boundary.set_delta_s(kPathBoundsDeciderResolution);
  fallback_path_boundary.set_label("fallback");
  candidate_path_boundaries.emplace_back(fallback_path_boundary);
  ...
  if (plan_pull_over_path) {
    ...
    if (!ret.ok()) {
      ...
      PathBoundary pullover_path_boundary;
      for (size_t i = 0; i < pull_over_path_bound.size(); ++i) {
        auto* pull_over_boundary = pullover_path_boundary.add_boundary();
        pull_over_boundary->set_left_l(std::get<2>(pull_over_path_bound[i]));
        pull_over_boundary->set_right_l(std::get<1>(pull_over_path_bound[i]));
      }
      pullover_path_boundary.set_start_s(std::get<0>(pull_over_path_bound[0]));
      pullover_path_boundary.set_delta_s(kPathBoundsDeciderResolution);
      pullover_path_boundary.set_label("regular/pullover");
      candidate_path_boundaries.emplace_back(pullover_path_boundary);
  ...
  if (FLAGS_enable_smarter_lane_change &&
    ...
    PathBoundary lanechange_path_boundary;
    for (size_t i = 0; i < lanechange_path_bound.size(); ++i) {
      auto* lanechange_boundary = lanechange_path_boundary.add_boundary();
      lanechange_boundary->set_left_l(std::get<2>(lanechange_path_bound[i]));
      lanechange_boundary->set_right_l(std::get<1>(lanechange_path_bound[i]));
    }
    lanechange_path_boundary.set_start_s(std::get<0>(lanechange_path_bound[0]));
    lanechange_path_boundary.set_delta_s(kPathBoundsDeciderResolution);
    lanechange_path_boundary.set_label("regular/lanechange");
    candidate_path_boundaries.emplace_back(lanechange_path_boundary);
  ...
  for (const auto& lane_borrow_info : lane_borrow_info_list) {
    ...
    PathBoundary regular_path_boundary;
    for (size_t i = 0; i < regular_path_bound.size(); ++i) {
      auto* regular_boundary = regular_path_boundary.add_boundary();
      regular_boundary->set_left_l(std::get<2>(regular_path_bound[i]));
      regular_boundary->set_right_l(std::get<1>(regular_path_bound[i]));
    }
    ...
    regular_path_boundary.set_start_s(std::get<0>(regular_path_bound[0]));
    regular_path_boundary.set_delta_s(kPathBoundsDeciderResolution);
    regular_path_boundary.set_label( absl::StrCat("regular/", path_label, "/", borrow_lane_type));
    regular_path_boundary.set_blocking_obstacle_id(blocking_obstacle_id);
    candidate_path_boundaries.emplace_back(regular_path_boundary);
  }
  ...
  while ((search_backward && idx >= 0 &&
    ...
    if (is_feasible_window) {
      ...
      const double pull_over_l =
          std::get<1>(pull_over_point) + pull_over_space_width / 2.0 + FLAGS_pull_over_offset;
...
void PathBoundsDecider::RemoveRedundantPathBoundaries(
  ...
  for (const auto& path_boundary : *candidate_path_boundaries) {
    if (path_boundary.label().find("left") != std::string::npos) {
      is_left_exist = true;
      TransfromPathboundaryTopair(path_boundary,left_boundary);
    }
    if (path_boundary.label().find("right") != std::string::npos) {
      is_right_exist = true;
      TransfromPathboundaryTopair(path_boundary,right_boundary);
    }
  }
...
void PathBoundsDecider::TransfromPathboundaryTopair(PathBoundary path_boundary , std::vector<std::pair<double, double>>& pair_boundary)
{
  std::pair<double, double> bound_pair;
  for(auto bound : path_boundary.boundary())
  {
    bound_pair.first = bound.right_l();
    bound_pair.second = bound.left_l();
    pair_boundary.emplace_back(bound_pair);
  }
}
