...
Status SpeedDecider::MakeObjectDecision(
  ...
  for (const auto* obstacle : path_decision->obstacles().Items()) {
    ...
    bool reverse_obs_flag = false;   //opposing and road-occupying obstacle
    ...
    auto adc_sl_info = reference_line_->ToFrenetFrame(init_point_); 
    adc_frenet_s_ = adc_sl_info.first[0];
    reference_line_->GetLaneWidth(adc_frenet_s_, &lane_left_width,
                                   &lane_right_width);
    if (obstacle->PerceptionSLBoundary().start_l() > 0 && obstacle->PerceptionSLBoundary().start_l() < lane_left_width
        && !obstacle->IsStatic() && obstacle_speed < 0)
    {
      if (obstacle->PerceptionSLBoundary().start_s() < adc_sl_boundary_.end_s())
      {
        continue;
      }
      reverse_obs_flag = true;      
      ObjectDecisionType yield_decision;
      if (CreateYieldDecision(*mutable_obstacle, &yield_decision,reverse_obs_flag)) {
...
bool SpeedDecider::CreateYieldDecision(
    const Obstacle& obstacle, ObjectDecisionType* const yield_decision, bool const reverse_obs_flag) const {
  PerceptionObstacle::Type obstacle_type = obstacle.Perception().type();
  ...
  const double obstacle_speed =
      common::math::Vec2d::CreateUnitVec2d(init_point_.path_point().theta())
          .InnerProd(Vec2d(velocity.x(), velocity.y()));
  yield_distance = obstacle_speed > 0 ? yield_distance : FLAGS_reverse_yield_distance;
  double dis_ = obstacle.PerceptionSLBoundary().start_s() - adc_sl_boundary_.end_s();
  double reference_line_fence_s;
  double yield_distance_s;
  if (reverse_obs_flag)
  {
    yield_distance_s = 
      std::max(-dis_, -yield_distance);
    reference_line_fence_s =
      adc_sl_boundary_.end_s() + dis_ + yield_distance_s;
  }
  else{
     yield_distance_s = 
