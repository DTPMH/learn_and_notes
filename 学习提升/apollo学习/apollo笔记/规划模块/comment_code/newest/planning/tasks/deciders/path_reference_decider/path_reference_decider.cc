...
void PathReferenceDecider::PathBoundToLineSegments(
  ...
  const size_t path_bound_size = path_bound.boundary_size();
  std::vector<LineSegment2d> cur_left_bound_segments;
  std::vector<LineSegment2d> cur_right_bound_segments;
  std::vector<std::pair<double, double>> path_bound_points; 
  std::pair<double,double> path_bound_pair;    
  for(auto bound : path_bound.boundary())
  {
    path_bound_pair.first = bound.right_l();
    path_bound_pair.second = bound.left_l();
    path_bound_points.push_back(path_bound_pair);
  }
...
int PathReferenceDecider::IsPointWithinPathBounds(
  ...
  const double start_s = path_bound.start_s();
  const double delta_s = path_bound.delta_s();
  const int path_bound_size = path_bound.boundary_size();
  ...
  if (idx_after == 0) {
    ...
    if (path_bound.boundary(idx_before).right_l() <= point_sl.l() &&
        path_bound.boundary(idx_before).left_l() >= point_sl.l() &&
        path_bound.boundary(idx_after).right_l() <= point_sl.l() &&
        path_bound.boundary(idx_after).left_l()  >= point_sl.l()) {
      return idx_after;
    }
  }
...
void PathReferenceDecider::EvaluatePathReference(
  ...
  size_t idx;
  for (idx = 0; idx < static_cast<size_t>(path_bound.boundary_size()); ++idx) {
    // relative s
