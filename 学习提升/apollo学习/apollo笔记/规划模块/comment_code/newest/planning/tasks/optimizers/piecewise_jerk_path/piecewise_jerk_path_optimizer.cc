...
common::Status PiecewiseJerkPathOptimizer::Process(
  ...
  const auto& pjp_optimizer_config =
  ...
  const auto& config =
      reference_line_info_->IsChangeLanePath()
          ? pjp_optimizer_config.lane_change_path_config()
          // : pjp_optimizer_config.pull_over_path_config();
          : (pull_over_status.has_position()
          ? pjp_optimizer_config.pull_over_path_config()
          : pjp_optimizer_config.default_path_config());

  // AINFO << "===> pull_over_status: " << pull_over_status.has_position();
  ...
  for (const auto& path_boundary : path_boundaries) {
    size_t path_boundary_size = path_boundary.boundary_size();
    ...
    std::pair<double,double>path_bounds_pair;
    std::vector<std::pair<double,double> > path_boundary_vec;
    for(auto bound : path_boundary.boundary())
    {
      path_bounds_pair.first = bound.right_l();
      path_bounds_pair.second = bound.left_l();
      path_boundary_vec.push_back(path_bounds_pair);
    }
    bool res_opt =
        OptimizePath(init_frenet_state, end_state, std::move(path_reference_l),
                     path_reference_size, path_boundary.delta_s(),
                     is_valid_path_reference, path_boundary_vec,
