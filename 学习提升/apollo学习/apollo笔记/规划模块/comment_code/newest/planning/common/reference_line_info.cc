...
bool ReferenceLineInfo::Init(const std::vector<const Obstacle*>& obstacles) {
  ...
  for (const auto& speed_bump : map_path.speed_bump_overlaps()) {
    ...
    reference_line_.AddSpeedLimit(speed_bump.start_s - 3.0,
                                  speed_bump.end_s + 3.0,
...
bool ReferenceLineInfo::IsTrigEmergencyLight(PlanningContext* planning_context, 
                                             MainDecision* main_decision , double dis_, PerceptionObstacle::Type obs_type) const{
  //when driving_mode is not AUTO_DRIVE,forbid trigger emergency_light
  if(vehicle_state_.driving_mode() !=Chassis::DrivingMode::Chassis_DrivingMode_COMPLETE_AUTO_DRIVE)
  {
    return false;
  }
  //when ego speed lower 0.5 and distance to stop_wall lower 0.5, consider maybe need triggle emergency_light
  if((vehicle_state_.linear_velocity() > FLAGS_min_stop_speed) || (dis_ > FLAGS_min_stop_distance)){
    //when ego start move，forbid emergency_light
    if(planning_context->emengency_flag_){
      planning_context->emengency_flag_ = false;
    }
    return false;
  }
  //if stop beacuse obstacle block the ego ,check it's type
  if(main_decision->stop().reason_code() == StopReasonCode::STOP_REASON_OBSTACLE){
    //if obs type is PEDESTRIAN and BICYCLE ,consider this is temporary park 
    if(obs_type == PerceptionObstacle::PEDESTRIAN ||
       obs_type == PerceptionObstacle::BICYCLE){
        return false;
    }
    //Avoid false triggering caused by traffic jam 
    if(path_data().path_label().find("self") != std::string::npos){
      //When there are other obstacles in front of the obstacle, it's value is true. 
      //I think this can avoid traffic jam
      if(planning_context->block == true){
        return false;
      }
    }
    //start time when ego stop
    if(!planning_context->emengency_flag_){
      planning_context->wait_time = Clock::NowInSeconds();
    }
    return true;
  }
  //stop for emergency situation  
  else if(main_decision->stop().reason_code() == StopReasonCode::STOP_REASON_REFERENCE_END
        || main_decision->stop().reason_code() == StopReasonCode::STOP_REASON_SIDEPASS_SAFETY
        || main_decision->stop().reason_code() == StopReasonCode::STOP_REASON_LANE_CHANGE_URGENCY){
          if(!planning_context->emengency_flag_){
            planning_context->wait_time = Clock::NowInSeconds();
            }
            return true;
          }
  // other situation consider no triger takeover                                          
  return false; 
}
...
void ReferenceLineInfo::ExportDecision(
  ...
  double DisToObs;
  PerceptionObstacle::Type obs_type;
  MakeDecision(decision_result, planning_context, DisToObs, obs_type);
  ...
  if (main_decision->has_stop()) {
    main_decision->mutable_stop()->set_change_lane_type(Lanes().PreviousAction());
    if(IsTrigEmergencyLight(planning_context, main_decision, DisToObs, obs_type)){
      planning_context->emengency_flag_ = true;
    }
    // the stop time should greater then 10s (in avoid temporary park)
    if(planning_context->emengency_flag_){
      double current_time = Clock::NowInSeconds();
      double duration = current_time - planning_context->wait_time;
      if(duration > FLAGS_emergency_delay_time)
        decision_result->mutable_vehicle_signal()->set_emergency_light(true);
    }
...
void ReferenceLineInfo::MakeDecision(DecisionResult* decision_result,
                                     PlanningContext* planning_context , double& s, PerceptionObstacle::Type& obs_type) const {
  ...
  // check stop decision
  int error_code = MakeMainStopDecision(decision_result , s , obs_type);
  if (error_code < 0) {
...
int ReferenceLineInfo::MakeMainStopDecision(
    DecisionResult* decision_result , double& s , PerceptionObstacle::Type& obs_type) const {
  ...
  if (stop_obstacle != nullptr) {
    ...
    s = min_stop_line_s - AdcSlBoundary().end_s();
    ...
    obs_type = stop_obstacle->Perception().type();
    return 1;
  }
  return 0;
}
