...
class ReferenceLineInfo {
 public:
  ...
  void SetFallbackAcc(double fallback_acc) { fallback_acc_ = fallback_acc; }
  double GetFallbackAcc() const { return fallback_acc_; }
 ...
 private:
  ...
  bool IsTrigEmergencyLight(PlanningContext* planning_context, 
  ...
  int MakeMainStopDecision(DecisionResult* decision_result , double& s , PerceptionObstacle::Type& obs_type) const;
 ...
 private:
  ...
  /**
   * @brief Using for generating fallback speed point
   */
  double fallback_acc_ = 0.0;
