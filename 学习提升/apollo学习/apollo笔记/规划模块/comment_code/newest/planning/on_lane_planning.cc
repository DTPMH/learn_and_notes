...
Status OnLanePlanning::Plan(
  ...
  if (FLAGS_enable_record_debug) {
    ...
    if (FLAGS_enable_record_debug) {
      ...
      //add path_boundary to planning_data
      if(!(frame_->reference_line_info().empty()))
      {
        for(const auto& reference_line_info : frame_->reference_line_info())
        {
          if(!(reference_line_info.GetCandidatePathBoundaries().empty()))
          {
            for(const auto& boundary : reference_line_info.GetCandidatePathBoundaries())
            {
              auto* path_boundary = ptr_debug->mutable_planning_data()->add_path_boundary();
              ...
              const auto& boundarys = boundary.boundary();
              int i = 0;
              double cur_s = 0.0;
              if(boundarys.empty()) break;
              for (const auto& bound : boundarys) 
              {
                auto* boundary_ = path_boundary->add_boundary();
                boundary_->set_right_l(bound.right_l());
                boundary_->set_left_l(bound.left_l());
                cur_s = boundary.start_s()+i*boundary.delta_s();
                auto pathpoint = reference_line_info.reference_line().GetReferencePoint(cur_s);
                common::PathPoint reference_point = pathpoint.ToPathPoint(cur_s);
                boundary_->mutable_referencepoint()->CopyFrom(reference_point);
                i++;
              }
            }
          }
