...
bool DiscretePointsReferenceLineSmoother::Smooth(
  ...
  for (const auto& anchor_point : anchor_points_) {
    raw_point2d.emplace_back(anchor_point.path_point.x(),
                             anchor_point.path_point.y());//将固定点放置到raw_point2d中
    anchorpoints_lateralbound.emplace_back(anchor_point.lateral_bound);//将l的bound放入
  }
  ...
  NormalizePoints(&raw_point2d);//根据固定点的第一个点归一化所有点（为啥）
  ...
  switch (smoothing_method) {
    case DiscretePointsSmootherConfig::COS_THETA_SMOOTHING:
      status = CosThetaSmooth(raw_point2d, anchorpoints_lateralbound,
                              &smoothed_point2d);
      break;
    case DiscretePointsSmootherConfig::FEM_POS_DEVIATION_SMOOTHING://true
  ...
  DeNormalizePoints(&smoothed_point2d);//反归一化，方法同上

  std::vector<ReferencePoint> ref_points;
  GenerateRefPointProfile(raw_reference_line, smoothed_point2d, &ref_points);//计算这些引导线点的参数
...
bool DiscretePointsReferenceLineSmoother::FemPosSmooth(
  ...
  std::vector<double> opt_x;
  std::vector<double> opt_y;
  bool status = smoother.Solve(raw_point2d, box_bounds, &opt_x, &opt_y);//根据QP问题求解
  ...
  size_t point_size = opt_x.size();
  for (size_t i = 0; i < point_size; ++i) {
    ptr_smoothed_point2d->emplace_back(opt_x[i], opt_y[i]);//将结果放入ptr_smoothed_point2d
  }
