...
bool ReferenceLine::Stitch(const ReferenceLine& other) {//other:之前的参考线
  if (other.reference_points().empty()) {
    AWARN << "The other reference line is empty.";
    return true;
  }
  auto first_point = reference_points_.front();//本次参考线的第一个点
  common::SLPoint first_sl;
  if (!other.XYToSL(first_point, &first_sl)) {//第一个点到之前参考线的投影
    AWARN << "Failed to project the first point to the other reference line.";
    return false;
  }
  bool first_join = first_sl.s() > 0 && first_sl.s() < other.Length();

  auto last_point = reference_points_.back();//本参考线最后一个点
  common::SLPoint last_sl;
  if (!other.XYToSL(last_point, &last_sl)) {//到之前参考线的投影值
  ...
  if (first_join) {
    if (first_sl.l() > kStitchingError) {//L值太大，不能缝合//为什么大于0.1就不行了
    ...
    reference_points_.insert(reference_points_.begin(), other_points.begin(),
                             other_points.begin() + start_i);//在参考线之前插入之前的参考线的点
  }
  if (last_join) {
...
bool ReferenceLine::Segment(const common::math::Vec2d& point,
  ...
  common::SLPoint sl;
  if (!XYToSL(point, &sl)) {//point投影到参考线中距离最近的点的sl值
...
bool ReferenceLine::Segment(const double s, const double look_backward,
                            const double look_forward) {
  const auto& accumulated_s = map_path_.accumulated_s();//每个点的s值，起始点s值为0

  // inclusive
  auto start_index =
      std::distance(accumulated_s.begin(),
                    std::lower_bound(accumulated_s.begin(), accumulated_s.end(),
                                     s - look_backward));//返回车后面限制值的参考点的s值
...
ReferenceLine::ToFrenetFrame(const common::TrajectoryPoint& traj_point) const {
  ...
  common::SLPoint sl_point;
  XYToSL(traj_point.path_point(), &sl_point);//到引导线上的投影值，s,l

  std::array<double, 3> s_condition;
  std::array<double, 3> l_condition;
  ReferencePoint ref_point = GetReferencePoint(sl_point.s());//得到光滑后的参考线上对应该s的ReferencePoint(map_point,kppa,dkppa)
...
ReferencePoint ReferenceLine::GetReferencePoint(const double s) const {
  ...
  return InterpolateWithMatchedIndex(p0, s0, p1, s1, interpolate_index);//根据匹配的point插值获得参考线上一个点
}
...
ReferencePoint ReferenceLine::InterpolateWithMatchedIndex(
  ...
  const double kappa = common::math::lerp(p0.kappa(), s0, p1.kappa(), s1, s);//求解s处的曲率
  const double dkappa = common::math::lerp(p0.dkappa(), s0, p1.dkappa(), s1, s);//求解s处的曲率的导数
...
bool ReferenceLine::GetSLBoundary(const common::math::Box2d& box,
  ...
  std::vector<common::math::Vec2d> corners;
  box.GetAllCorners(&corners);//得到四个角的坐标

  // The order must be counter-clockwise
  std::vector<SLPoint> sl_corners;
  for (const auto& point : corners) {//遍历这个点
    SLPoint sl_point;
    if (!XYToSL(point, &sl_point)) {//得到这四个点到引导线的投影值s,l
      AERROR << "Failed to get projection for point: " << point.DebugString()
             << " on reference line.";
      return false;
    }
    sl_corners.push_back(std::move(sl_point));//将投影值s，l放入sl_corners中
  }
  ...
  for (size_t i = 0; i < corners.size(); ++i) {
    ...
    const auto p_mid = (p0 + p1) * 0.5;//求两个角点的中点
    SLPoint sl_point_mid;
    if (!XYToSL(p_mid, &sl_point_mid)) {//获得该中点的投影值
    ...
    *sl_boundary->add_boundary_point() = sl_corners[index0];
    ...
    if (v0.CrossProd(v1) < 0.0) {//三个点不在一条线上
      *sl_boundary->add_boundary_point() = sl_point_mid;
    }
  }
//遍历这些点，找到起始点s,end_s,start_l,end_l
