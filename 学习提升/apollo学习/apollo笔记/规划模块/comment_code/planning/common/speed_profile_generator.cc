...
SpeedData SpeedProfileGenerator::GenerateFallbackSpeed(
    const EgoInfo* ego_info, const double fallback_acc, const double stop_distance) {
  ...
  if (init_v <= 0.0 && init_a <= 0.0) {
    ...
    speed_data.AppendSpeedPoint(0.0, 0.0, 0.0, fallback_acc, 0.0);
    FillEnoughSpeedPoints(&speed_data, fallback_acc);
    return speed_data;
  }
  ...
  double delta_t = FLAGS_fallback_time_unit;//0.1
  double total_time = FLAGS_fallback_total_time;//3.0
...
void SpeedProfileGenerator::FillEnoughSpeedPoints(SpeedData* const speed_data, const double fallback_acc) {
  const SpeedPoint& last_point = speed_data->back();
  if (last_point.t() >= FLAGS_fallback_total_time) {//3.0
    return;
  }
  for (double t = last_point.t() + FLAGS_fallback_time_unit;
       t < FLAGS_fallback_total_time; t += FLAGS_fallback_time_unit) {
    speed_data->AppendSpeedPoint(last_point.s(), t, 0.0, fallback_acc, 0.0);
  }
}
