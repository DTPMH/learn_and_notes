...
std::list<std::unique_ptr<Obstacle>> Obstacle::CreateObstacles(
  ...
  for (const auto& prediction_obstacle : predictions.prediction_obstacle()) {//遍历所有预测到的障碍物
    if (!IsValidPerceptionObstacle(prediction_obstacle.perception_obstacle())) {//检查障碍物的长宽高，以及外围点的坐标是否合法
    ...
    if (prediction_obstacle.trajectory().empty()) {//如果该障碍物的轨迹为空
      obstacles.emplace_back(
          new Obstacle(perception_id, prediction_obstacle.perception_obstacle(),
                       prediction_obstacle.priority().priority(),
                       prediction_obstacle.is_static()));//根据预测的障碍物信息构建障碍物实例指针
      continue;
    }

    int trajectory_index = 0;
    for (const auto& trajectory : prediction_obstacle.trajectory()) {//将动态障碍物的每个轨迹（可能有多个预测轨迹，如路口区等）
      bool is_valid_trajectory = true;
      for (const auto& point : trajectory.trajectory_point()) {//检测轨迹点是否格式错误
      ...
      obstacles.emplace_back(
          new Obstacle(obstacle_id, prediction_obstacle.perception_obstacle(),
                       ...
                       prediction_obstacle.is_static()));//将每条轨迹都作为一个障碍物放入obstacles中
      ++trajectory_index;
    }//障碍物轨迹点检查结束
  }//所有障碍物处理结束
  return obstacles;//返回构建的障碍物指针列表
}
...
bool Obstacle::BuildTrajectoryStBoundary(const ReferenceLine& reference_line,
  ...
  for (int i = 1; i < trajectory_points.size(); ++i) {//遍历障碍物轨迹点
...
void Obstacle::CheckLaneBlocking(const ReferenceLine& reference_line) {
  ...
  if (sl_boundary_.start_l() * sl_boundary_.end_l() < 0.0) {//该障碍物压线，因此肯定会挡
    is_lane_blocking_ = true;
    return;
  }

  const double driving_width = reference_line.GetDrivingWidth(sl_boundary_);//得到不算该障碍物的可通行区域
  auto vehicle_param = common::VehicleConfigHelper::GetConfig().vehicle_param();

  if (reference_line.IsOnLane(sl_boundary_) &&
      driving_width <
          vehicle_param.width() + FLAGS_static_obstacle_nudge_l_buffer) {//0.3
    is_lane_blocking_ = true;
    return;
  }
