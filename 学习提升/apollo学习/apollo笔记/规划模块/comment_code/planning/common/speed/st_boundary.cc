...
bool STBoundary::GetUnblockSRange(const double curr_time, double* s_upper,
  ...
  *s_lower = 0.0;
  if (curr_time < min_t_ || curr_time > max_t_) {//当前时间小该障碍物的最小时间，或者大于该障碍物的最大时间，返回0-200
    return true;
  }

  size_t left = 0;
  size_t right = 0;
  //找到在当前时间处于当前障碍物的两个st_boundary的index,即：left-bound cur_time  right-bound(--| t |--) 
  if (!GetIndexRange(lower_points_, curr_time, &left, &right)) {//lower_points_下面的点
  ...
  const double r =
      (left == right
           ...
           : (curr_time - upper_points_[left].t()) /
                 (upper_points_[right].t() - upper_points_[left].t()));//计算比率
  ...
  double lower_cross_s =
      ...
      r * (lower_points_[right].s() - lower_points_[left].s());//计算该时刻的上下边界

  if (boundary_type_ == BoundaryType::STOP ||
      boundary_type_ == BoundaryType::YIELD ||
      boundary_type_ == BoundaryType::FOLLOW) {//如果保持在后面的话，则s_upper等于障碍物的下边界
    *s_upper = lower_cross_s;
  } else if (boundary_type_ == BoundaryType::OVERTAKE) {//超车的话，则s_lower为障碍物上边界
...
bool STBoundary::GetBoundarySRange(const double curr_time, double* s_upper,
  ...
  size_t left = 0;
  size_t right = 0;
  if (!GetIndexRange(lower_points_, curr_time, &left, &right)) {//得到当前时间处于的时间向量
    AERROR << "Fail to get index range.";
    return false;
  }
  const double r =
  ...
  *s_upper = upper_points_[left].s() +
             r * (upper_points_[right].s() - upper_points_[left].s());//根据时间差值得到边界值
  *s_lower = lower_points_[left].s() +
             r * (lower_points_[right].s() - lower_points_[left].s());//根据时间差值得到下边界值
