...
bool ReferenceLineInfo::Init(const std::vector<const Obstacle*>& obstacles) {
  const auto& param = VehicleConfigHelper::GetConfig().vehicle_param();//得到车辆参数
  // stitching point
  const auto& path_point = adc_planning_point_.path_point();//开始规划的点
  Vec2d position(path_point.x(), path_point.y());
  Vec2d vec_to_center(
      (param.front_edge_to_center() - param.back_edge_to_center()) / 2.0,
      (param.left_edge_to_center() - param.right_edge_to_center()) / 2.0);//车后轮到中心的向量（2.7,0）
  Vec2d center(position + vec_to_center.rotate(path_point.theta()));//计算开始规划点中心点的坐标
  Box2d box(center, path_point.theta(), param.length(), param.width());//根据朝向以及长度计算box的朝向，以及box的四个角的坐标
  ...
  Box2d vehicle_box(vehicle_center, vehicle_state_.heading(), param.length(),
                    param.width());
  //计算此时车的朝向与box
//计算车的box的四个点在引导线上的sl投影值计算start_l,end_l,start_s,end_s将其放入adc_sl_boundary_中
  if (!reference_line_.GetSLBoundary(box, &adc_sl_boundary_)) {
    AERROR << "Failed to get ADC boundary from box: " << box.DebugString();
    return false;
  }
//将该参考线中与各种类型车道相交区域的第一个点，并放入first_encounter_overlaps_中
//并根据start_s对其排序
  InitFirstOverlaps();
  ...
  static constexpr double kOutOfReferenceLineL = 10.0;  // L值超过10m
  ...
  is_on_reference_line_ = reference_line_.IsOnLane(adc_sl_boundary_);//检测车的box是否处于该引导线的lane中
  ...
  if (!AddObstacles(obstacles)) {//将障碍物（st）边界加入参考线信息中
    AERROR << "Failed to add obstacles to reference line";
    return false;
  }

  const auto& map_path = reference_line_.map_path();
  for (const auto& speed_bump : map_path.speed_bump_overlaps()) {//增加速度限制
    ...
    reference_line_.AddSpeedLimit(speed_bump.start_s - 3.0,
                                  ...
                                  FLAGS_speed_bump_speed_limit);//start_S,end_S，speed_limit
  }

  SetCruiseSpeed(FLAGS_default_cruise_speed);//5.0，赋值

  // set lattice planning target speed limit;
  SetLatticeCruiseSpeed(FLAGS_default_cruise_speed);//5.0
...
bool ReferenceLineInfo::GetFirstOverlap(
  ...
  const double start_s = adc_sl_boundary_.end_s();//车在引导线上的四个投影点
...
void ReferenceLineInfo::InitFirstOverlaps() {
  const auto& map_path = reference_line_.map_path();//得到该引导线上（收缩后）的点
  // clear_zone
...
//得到该障碍物在该引导线上的slboundary(start_s-end_s,start_l-end_l)
  ...
  mutable_obstacle->CheckLaneBlocking(reference_line_);//检查静态障碍物是否会阻挡lane,is_lane_blocking_为true或false
  //检查该障碍物是否将该引导线阻挡（endl*starts<0||drive_width<pass_width）
  ...
  if (IsIrrelevantObstacle(*mutable_obstacle)) {//检测障碍物是否相关（车后为true,车前超出引导线为true）,并为其增加决策
    ...
    mutable_obstacle->BuildReferenceLineStBoundary(reference_line_,
                                                   adc_sl_boundary_.start_s());//生成障碍物的stboundary
...
bool ReferenceLineInfo::AddObstacles(
  ...
  if (FLAGS_use_multi_thread_to_add_obstacles) {//false
    ...
    for (const auto* obstacle : obstacles) {//遍历障碍物
      if (!AddObstacle(obstacle)) {//计算障碍物的st_boundary并将其放入path_decision_中
        AERROR << "Failed to add obstacle " << obstacle->Id();
        return false;
      }
    }
...
bool ReferenceLineInfo::IsIrrelevantObstacle(const Obstacle& obstacle) {
  ...
  if (is_on_reference_line_ && !IsChangeLanePath() &&
      obstacle_boundary.end_s() < adc_sl_boundary_.end_s() &&//adc_sl_boundary_是车在引导线上的投影
...
bool ReferenceLineInfo::CombinePathAndSpeedProfile(
  ...
  for (double cur_rel_time = 0.0; cur_rel_time < speed_data_.TotalTime();
    ...
    common::PathPoint path_point =
        path_data_.GetPathPointWithPathS(speed_point.s());//根据s值计算路径点
    ...
    //将路径点，速度，加速度，时间放入trajectory_point中
  }//根据时间遍历speed点，计算该速度点对应的path point,将其放入trajectory_point中
  return true;
}
...
bool ReferenceLineInfo::IsChangeLanePath() const {
  return !Lanes().IsOnSegment();//当车处于该passage时为true
}
...
void ReferenceLineInfo::ExportDecision(
  ...
  MakeDecision(decision_result, planning_context);//遍历障碍物查看是否要停车
  ExportVehicleSignal(decision_result->mutable_vehicle_signal());//转向灯
...
int ReferenceLineInfo::MakeMainStopDecision(
  ...
  for (const auto* obstacle : path_decision_.obstacles().Items()) {//遍历障碍物，查看是否有停止决策的障碍物
  ...
  if (stop_obstacle != nullptr) {
    ...
    if(min_stop_line_s < adc_sl_boundary_.end_s())
    {
      auto* EmergencyLight = decision_result->mutable_vehicle_signal();
      EmergencyLight->set_emergency_light(true);
    }
    return 1;
  }
