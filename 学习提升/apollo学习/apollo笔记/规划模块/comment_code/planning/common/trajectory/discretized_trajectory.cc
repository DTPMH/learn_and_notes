...
size_t DiscretizedTrajectory::QueryLowerBoundPoint(const double relative_time,
                                                   const double epsilon) const {//epsilon==1e-5
  ACHECK(!empty());
//相当于this指针
  if (relative_time >= back().relative_time()) {//如果dt大于上一步轨迹的最后一个点的relative_time,则说明当前时间已经超出了上一步轨迹的最后一个点
    return size() - 1;//直接返回最后一个点就可以
  }
  auto func = [&epsilon](const TrajectoryPoint& tp,
                         const double relative_time) {
    return tp.relative_time() + epsilon < relative_time;
  };
  auto it_lower = std::lower_bound(begin(), end(), relative_time, func);//根据relative_time排序
  return std::distance(begin(), it_lower);//获得距离该时间点最近的上一步路径点
}
