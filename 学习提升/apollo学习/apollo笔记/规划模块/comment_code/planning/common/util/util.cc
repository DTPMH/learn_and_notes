...
bool CheckInsidePnCJunction(const ReferenceLineInfo& reference_line_info) {
  const double adc_front_edge_s = reference_line_info.AdcSlBoundary().end_s();//车头
  const double adc_back_edge_s = reference_line_info.AdcSlBoundary().start_s();//车尾

  hdmap::PathOverlap pnc_junction_overlap;
  reference_line_info.GetPnCJunction(adc_front_edge_s, &pnc_junction_overlap);
  if (pnc_junction_overlap.object_id.empty()) {//如果车头不在junction区域，则返回false
    return false;
  }
  ...
  return distance_adc_pass_intersection < kIntersectionPassDist;//车超出junction 2m 时为false
}
