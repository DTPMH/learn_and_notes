...
//检查配置信息
  ...
  injector_->history()->Clear();//清除规划历史，只执行一次
...
//获取引导线reference_lines，以及对应的routesegments（直接复制）
...
//根据汽车速度计算前向距离(速度大于22.5m/s时为250m，其他为180m)
  auto forward_limit =
...
//设置dead_end_point_，导航点(routing 请求中发出)
...
//目的是根据前一步轨迹与规划时间，计算本次规划的起始点
//匹配点 前20个点---->（当前时间+规划时间）对应的点（通过设置频率计算）  
...
//调用frame构造函数，将形参数据信息信息传入,实例化frame类
//收缩引导线，根据设定的前行距离与后向距离，对其进行收缩，并将收缩的点放入reference_points_以及map_path中
//构建障碍物信息(将障碍物的id和obstacle放入obstacles_(frame类中)={id，obstacle})，并检查障碍物是否会和汽车相撞
...
//挑出不符合交通规则的导航点
//reference_line_info_保存每条引导线对应的汽车位置（固定），开始规划点（固定）
//以及引导线，lansegment,以及障碍物的slboundary,stboundary
  for (auto& ref_line_info : *frame_->mutable_reference_line_info()) {//遍历所有引导线info
    TrafficDecider traffic_decider;
    traffic_decider.Init(traffic_rule_configs_);//config赋值
    auto traffic_status =
        traffic_decider.Execute(frame_.get(), &ref_line_info, injector_);
        //并根据交通规则设定虚拟stop_wall障碍物
...
//pangminghui change to add path_boundary to ptr_trajectory_pb
  auto* planning_data = ptr_trajectory_pb->mutable_debug()->mutable_planning_data();
  ...
  for(const auto& reference_line_info : frame_.get()->reference_line_info())//遍历每条参考线
  {
    for(auto& boundary : reference_line_info.GetCandidatePathBoundaries())//每条参考线有多个候选边界，遍历每个候选边界
    {
      ...
      auto* path_boundary = planning_data->add_path_boundary();
      path_boundary->set_start_s(boundary.start_s());
      path_boundary->set_delta_s(boundary.delta_s());
      double curs = boundary.start_s();
      double ds = boundary.delta_s();
      //遍历该候选边界的boundary,将其放入
      int i = 0;
      for(auto edge : boundary.boundary())
      {
        auto* vec_bound = path_boundary->add_boundary();
        vec_bound->set_left_l(edge.second);
        vec_bound->set_right_l(edge.first);
        auto pathpoint = reference_line_info.reference_line().GetReferencePoint(curs+i*ds);
        common::PathPoint reference_point = pathpoint.ToPathPoint(curs+i*ds); 
        vec_bound->mutable_referencepoint()->set_x(reference_point.x());
        vec_bound->mutable_referencepoint()->set_y(reference_point.y());
        vec_bound->mutable_referencepoint()->set_theta(reference_point.theta());
        vec_bound->mutable_referencepoint()->set_kappa(pathpoint.kappa());
        vec_bound->mutable_referencepoint()->set_dkappa(pathpoint.dkappa());
        vec_bound->mutable_referencepoint()->set_s(curs+i*ds);
        i++;
      }
      path_boundary->set_label(boundary.label());
    }
  }
  AINFO<<"=====>on_lane_planning.cc"<<planning_data->DebugString();
  //end
  if (frame_->open_space_info().is_on_open_space_trajectory()) {//false
    ...
    const auto* best_ref_info = frame_->FindDriveReferenceLineInfo();//找到cost最小的参考线(就是最近的参考线，因为如果变道的话，cost会很大)
    ...
    const auto& best_ref_path = best_ref_info->path_data().discretized_path();//获得路径点
    std::copy(best_ref_path.begin() + 1, best_ref_path.end(),//将路径点拷贝到current_frame_planned_path中
              std::back_inserter(current_frame_planned_path));
    frame_->set_current_frame_planned_path(current_frame_planned_path);

    ptr_debug->MergeFrom(best_ref_info->debug());

    if (FLAGS_export_chart) {//true
    ...
    best_ref_info->ExportDecision(ptr_trajectory_pb->mutable_decision(),
                                  injector_->planning_context());//停车到达目标点处理
...
//轨迹点赋值函数
    last_publishable_trajectory_.reset(new PublishableTrajectory(
        current_time_stamp, best_ref_info->trajectory()));//将轨迹点赋给last_publishable_trajectory_

    ADEBUG << "current_time_stamp: " << current_time_stamp;

    last_publishable_trajectory_->PrependTrajectoryPoints(
        std::vector<TrajectoryPoint>(stitching_trajectory.begin(),
                                     stitching_trajectory.end() - 1));//将缝合轨迹放入last_publishable_trajectory_的开头

    last_publishable_trajectory_->PopulateTrajectoryProtobuf(ptr_trajectory_pb);//将轨迹点赋给ptr_trajectory_pb
