...
void TrafficLight::MakeDecisions(Frame* const frame,
  ...
  const auto& traffic_light_status =
      injector_->planning_context()->planning_status().traffic_light();//获取红绿灯状态

  const double adc_front_edge_s = reference_line_info->AdcSlBoundary().end_s();//车头位置在参考线上的s边界值
  ...
  const std::vector<PathOverlap>& traffic_light_overlaps =
      reference_line_info->reference_line().map_path().signal_overlaps();//参考线中的交通灯区域
  for (const auto& traffic_light_overlap : traffic_light_overlaps) {//遍历参考线中的交通灯length
    if (traffic_light_overlap.end_s <= adc_back_edge_s) {//车已经通过了该信号灯
      continue;
    }
    ...
    const double distance =
        common::util::DistanceXY(traffic_light_point, adc_position);//车与信号灯之间的距离
    const double s_distance = traffic_light_overlap.start_s - adc_front_edge_s;//两者之间的s距离
    ...
    if (s_distance >= 0 &&
        fabs(s_distance - distance) > kSDiscrepanceTolerance) {//灯在车前，且距离大于10m,没必要处理则跳过
    ...
    auto signal_color =
        frame->GetSignal(traffic_light_overlap.object_id).color();//得到交通灯信号
    ...
    if (stop_deceleration > config_.traffic_light().max_stop_deceleration()) {//当需要的减速度大于4时，则只能闯红灯了
