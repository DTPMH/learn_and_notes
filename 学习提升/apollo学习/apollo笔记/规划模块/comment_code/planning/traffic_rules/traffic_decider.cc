...
void TrafficDecider::BuildPlanningTarget(
  ...
  for (const auto *obstacle :
       reference_line_info->path_decision()->obstacles().Items()) {//遍历构建的所有障碍物（包括真实与刚刚构建的虚拟障碍物）
  ...
  if (min_s != std::numeric_limits<double>::infinity()) {
    ...
    stop_point.set_s(min_s - front_edge_to_center +
                     FLAGS_virtual_stop_wall_length / 2.0);//计算停下的s值
    reference_line_info->SetLatticeStopPoint(stop_point);//设置停止的Lattice点
  }
}
...
Status TrafficDecider::Execute(
  ...
  for (const auto &rule_config : rule_configs_.config()) {//遍历交通规则
    ...
    rule->ApplyRule(frame, reference_line_info);//根据红路灯以及参考线信息，生成决策信息
    ADEBUG << "Applied rule "
