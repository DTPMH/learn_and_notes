...
Status Crosswalk::ApplyRule(Frame* const frame,
  ...
  if (!FindCrosswalks(reference_line_info)) {//将引导线上的人行道区域放入crosswalk_overlaps_中
...
void Crosswalk::MakeDecisions(Frame* const frame,
  ...
  for (auto crosswalk_overlap : crosswalk_overlaps_) {
    ...
    if (adc_front_edge_s - crosswalk_overlap->end_s >
        config_.crosswalk().min_pass_s_distance()) {//1.0
    ...
    for (const auto* obstacle : path_decision->obstacles().Items()) {
      const double stop_deceleration = util::GetADCStopDeceleration(
          ...
          crosswalk_overlap->start_s);//根据车头与人行道之间的距离计算汽车完全停下需要的减速度
      ...
      if (stop && !is_on_lane &&
          crosswalk_overlap->start_s - adc_front_edge_s <=
              kStartWatchTimerDistance) {//40，如果针对该障碍物需要停止且该障碍物不在lane中，且车与人行道距离小于40m时
        ...
        if (obstacle_speed <= kMaxStopSpeed) {//0.3
          if (crosswalk_stop_timer[crosswalk_id].count(obstacle_id) < 1) {//之前没有停下了
            // add timestamp
            ...
            double stop_time = Clock::NowInSeconds() -
                               crosswalk_stop_timer[crosswalk_id][obstacle_id];//已经停了多长时间
            ADEBUG << "stop_time: obstacle_id[" << obstacle_id << "] stop_time["
                   << stop_time << "]";
            if (stop_time >= config_.crosswalk().stop_timeout()) {//4s
              stop = false;
            }
          }
        }//障碍物速度小于0.3处理结束
  ...
  for (auto crosswalk_to_stop : crosswalks_to_stop) {//对每个需要停的人行横道添加虚拟停止障碍物
    ...
    util::BuildStopDecision(virtual_obstacle_id, crosswalk_overlap->start_s,
                            config_.crosswalk().stop_distance(),//1.0
...
bool Crosswalk::CheckStopForObstacle(
  ...
  // check type(其他两种类型UNKNOWN_UNMOVABLE，VEHICLE不需要停止)
  ...
  const Polygon2d crosswalk_exp_poly =
      crosswalk_ptr->polygon().ExpandByDistance(
          config_.crosswalk().expand_s_distance());//2
  bool in_expanded_crosswalk = crosswalk_exp_poly.IsPointIn(point);

  if (!in_expanded_crosswalk) {//如果上述4各类型的障碍物不在扩展后的人行横道上，则不停止
    ADEBUG << "skip: obstacle_id[" << obstacle_id << "] type["
           << obstacle_type_name << "] crosswalk_id[" << crosswalk_id
  ...
  bool stop = false;
  if (obstacle_l_distance >= config_.crosswalk().stop_loose_l_distance()) {//8
    // (1) when obstacle_l_distance is big enough(>= loose_l_distance),
    //     STOP only if paths crosses
    if (is_path_cross) {//障碍物在引导线上有stboundary（表示该障碍物会变到该引导线上）
      stop = true;
      ADEBUG << "need_stop(>=l2): obstacle_id[" << obstacle_id << "] type["
             << obstacle_type_name << "] crosswalk_id[" << crosswalk_id << "]";
    }
  } else if (obstacle_l_distance <=
             config_.crosswalk().stop_strict_l_distance()) {//6
    if (is_on_road) {
      // (2) when l_distance <= strict_l_distance + on_road
      //     always STOP
      if (obstacle_sl_point.s() > adc_end_edge_s) {//障碍物在车前，需要停止
        stop = true;
  ...
  if (stop) {
    if (stop_deceleration >= config_.crosswalk().max_stop_deceleration()) {//6
      if (obstacle_l_distance > config_.crosswalk().stop_strict_l_distance()) {//6
