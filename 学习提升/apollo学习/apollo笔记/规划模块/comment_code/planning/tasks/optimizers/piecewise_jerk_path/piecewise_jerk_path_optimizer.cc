...
common::Status PiecewiseJerkPathOptimizer::Process(
    ...
    const common::TrajectoryPoint& init_point, const bool path_reusable,//false
    PathData* const final_path_data) {//final_path_data===引导线的path_data(mutable)
  // skip piecewise_jerk_path_optimizer if reused path
  if (FLAGS_enable_skip_path_tasks && path_reusable) {//（false&&false).不成立
    return Status::OK();
  }
  ...
  for (const auto& path_boundary : path_boundaries) {//遍历该引导线的boundary
    size_t path_boundary_size = path_boundary.boundary().size();//bound中s点的个数
    ...
    }//靠边停车的投影值计算，并将L值放入end_state[0]中

    // TODO(all): double-check this;
    // final_path_data might carry info from upper stream
    PathData path_data = *final_path_data;//final_path_data===引导线的path_data,mutable
    ...
    //如果之前该参考线存在path_data,则根据该数量进行优化，若是没有，全部为0
    size_t path_reference_size = reference_path_data.path_reference().size();//path_reference中存放的即该具体的路径点
//以下均跳过
    if (path_boundary.label().find("regular") != std::string::npos &&
...
FrenetFramePath PiecewiseJerkPathOptimizer::ToPiecewiseJerkPath(
  ...
  for (std::size_t i = 1; i < x.size(); ++i) {
    ...
    piecewise_jerk_traj.AppendSegment(dddl, delta_s);//根据运动学公式，以及delta_s计算L轨迹
  }

  std::vector<common::FrenetFramePoint> frenet_frame_path;
  double accumulated_s = 0.0;
  while (accumulated_s < piecewise_jerk_traj.ParamLength()) {//S的总长度
