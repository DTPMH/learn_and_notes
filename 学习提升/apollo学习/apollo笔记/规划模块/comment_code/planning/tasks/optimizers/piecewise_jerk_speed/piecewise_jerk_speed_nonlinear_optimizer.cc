...
Status PiecewiseJerkSpeedNonlinearOptimizer::Process(
  ...
  if (reference_line_info_->ReachedDestination()) {//车与目标点的距离小于0.05
    return Status::OK();
  }
...
Status PiecewiseJerkSpeedNonlinearOptimizer::OptimizeByQP(
  ...
  std::array<double, 3> init_states = {s_init_, s_dot_init_, s_ddot_init_};//设置初值
  ...
  piecewise_jerk_problem.set_weight_ddx(config.acc_weight());//2
  piecewise_jerk_problem.set_weight_dddx(config.jerk_weight());//3
...
Status PiecewiseJerkSpeedNonlinearOptimizer::OptimizeByNLP(
  ...
  if (config.use_warm_start()) {
    ...
    for (std::size_t i = 0; i < size; ++i) {//放入优化变量的初值
  ...
  if (FLAGS_use_smoothed_dp_guide_line) {//true
  ...
  ptr_interface->set_reference_speed(cruise_speed_);//巡航速度
