...
apollo::common::Status RuleBasedStopDecider::Process(
  ...
  //增加停止墙，并设定目标前5米范围为障碍物，设定停止决策
  //当靠近终点的时候，有一种特殊情况，最后一段生成的path非常短小于20m，提前5m停车。
...
void RuleBasedStopDecider::AddPathEndStop(
  ...
  if (!reference_line_info->path_data().path_label().empty() &&
      reference_line_info->path_data().frenet_frame_path().back().s() -
          ...
          FLAGS_short_path_length_threshold) {//如果路径的end-start小于20
    const std::string stop_wall_id =
        PATH_END_VO_ID_PREFIX + reference_line_info->path_data().path_label();//停止墙的id
    std::vector<std::string> wait_for_obstacles;
    util::BuildStopDecision(
        stop_wall_id,
...
void RuleBasedStopDecider::StopOnSidePass(
  ...
  if (path_data.path_label().find("self") != std::string::npos) {//直行道，直接返回
    check_clear = false;
...
bool RuleBasedStopDecider::CheckClearDone(
  ...
  // again
  if (adc_back_edge_s > stop_sl_point.s()) {//车超出目标点
    if (adc_start_l > -lane_right_width || adc_end_l < lane_left_width) {//车在lane内
      return true;
    }
  }
  return false;
