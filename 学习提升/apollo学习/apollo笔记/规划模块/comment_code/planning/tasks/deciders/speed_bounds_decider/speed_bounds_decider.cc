...
//判断每个障碍物的行驶方向，同向行驶返回该障碍物boundary的起始最低的s值
//逆向行驶返回0.0
...
double SpeedBoundsDecider::SetSpeedFallbackDistance(
  ...
  for (auto *obstacle : path_decision->obstacles().Items()) {
    ...
    if (left_bottom_point_s - right_bottom_point_s > kEpsilon) {//障碍物越来越近，相向行驶
      if (min_s_reverse > lowest_s) {
        min_s_reverse = lowest_s;
      }
    } else if (min_s_non_reverse > lowest_s) {//同向行驶
