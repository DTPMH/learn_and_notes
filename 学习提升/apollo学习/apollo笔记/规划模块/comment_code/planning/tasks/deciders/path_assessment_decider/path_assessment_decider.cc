...
Status PathAssessmentDecider::Process(
  ...
  // 3. Pick the optimal path.选择最优的路径
  //对合法路径进行排序(需加强C++排序知识)
  ...
  *(reference_line_info->mutable_path_data()) = valid_path_data.front();//将最短路径赋值
  reference_line_info->SetBlockingObstacle(
      valid_path_data.front().blocking_obstacle_id());//借道时获得
  const auto& end_time3 = std::chrono::system_clock::now();
  diff = end_time3 - end_time2;
  ADEBUG << "Time for optimal path selection: " << diff.count() * 1000
         << " msec.";

  reference_line_info->SetCandidatePathData(std::move(valid_path_data));//重新赋值SetCandidatePathData
...
bool PathAssessmentDecider::IsValidRegularPath(
  ...
  if (IsGreatlyOffReferenceLine(path_data)) {//20m
...
void PathAssessmentDecider::SetPathInfo(
  ...
  //将path_data的每个点初始化为：PointDecision contains (s, PathPointType::unknown, distance to closest obstacle).

  // 1. Label caution types, differently for side-pass or lane-change.
  //为path上的每个点打标签
  if (reference_line_info.IsChangeLanePath()) {//如果变道
  ...
  path_data->SetPathPointDecisionGuide(std::move(path_decision));//将path_decision赋值给path_point_decision_guide_
}
...
void PathAssessmentDecider::SetPathPointType(
  ...
  for (size_t i = 0; i < discrete_path.size(); ++i) {
    ...
    if (reference_line_info.reference_line().GetLaneWidth(
      ...
      if (is_lane_change_path) {//是否变道，根据车目前的sl_boundary与lane的宽度进行对比，对每个path点打标签，有INLANE，以及OUT_ON_FORWARD_LANE
        // For lane-change path, only transitioning part is labeled as
        // out-of-lane.
        if (ego_sl_boundary.start_l() > lane_left_width ||
            ego_sl_boundary.end_l() < -lane_right_width) {//车仍处于上一条路上
          ...
          std::get<1>((*path_point_decision)[i]) =
              PathData::PathPointType::IN_LANE;
                   ...
                   ego_sl_boundary.end_l() <
                       lane_left_width - back_to_inlane_extra_buffer) {//车已经处于下一条lane了，即变道结束了
