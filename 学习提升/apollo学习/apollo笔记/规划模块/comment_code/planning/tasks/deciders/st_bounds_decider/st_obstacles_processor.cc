...
Status STObstaclesProcessor::MapObstaclesToSTBoundaries(
  ...
  for (const auto& path_pt_info : path_data_.path_point_decision_guide()) {
    ...
    std::tie(path_pt_s, path_pt_type, std::ignore) = path_pt_info;//对path_pt_info接码分别赋值，
  ...
  for (const auto* obs_item_ptr : path_decision->obstacles().Items()) {//遍历每个障碍物
    // Sanity checks.
    ...
    //根据删减的边界点生成该障碍物的alternative_boundary
    ...
    if (obs_item_ptr->Id().find("KC") != std::string::npos) {//如果该障碍物是紧急车道，则不能使用削减后的boundary
    ...
    if (obs_ptr->Trajectory().trajectory_point().empty()) {
      ...
      if (std::get<0>(closest_stop_obstacle) == "NULL" ||
        ...
        closest_stop_obstacle =
            std::make_tuple(obs_ptr->Id(), boundary, obs_ptr);//获得边界s值最小的静态障碍物，即离车目前最近的障碍物
      }
    } else {
      // Obstacle is dynamic.
      //首先忽略在车后面的动态障碍物
      if (boundary.bottom_left_point().s() - adc_path_init_s_ <
              kSIgnoreThreshold &&//0.01
          boundary.bottom_left_point().t() > kTIgnoreThreshold) {//0.1
        // Ignore obstacles that are behind.
        // TODO(jiacheng): don't ignore if ADC is in dangerous segments.
        continue;
      }
    //之后设置动态障碍物的st_boundary
      obs_id_to_st_boundary_[obs_ptr->Id()] = boundary;
      obs_ptr->set_path_st_boundary(boundary);//给obs添加boundary
  ...
  if (std::get<0>(closest_stop_obstacle) != "NULL") {//有静态障碍物距离path很近时为obs的id
    ...
    closest_stop_obs_ptr->set_path_st_boundary(closest_stop_obs_boundary);//设置该障碍物的边界
  ...
  for (const auto* obs_item_ptr : path_decision->obstacles().Items()) {//忽略无关的障碍物
    Obstacle* obs_ptr = path_decision->Find(obs_item_ptr->Id());
...
bool STObstaclesProcessor::GetSBoundsFromDecisions(
  ...
  // Gather any possible change in st-boundary situations.
   //obs_t_edges_类型（每个障碍物在路径上的起始点与终点与时间）：
  //[true,起始时间，左下边界点的s,左上边界点的s,obs_id]
   //[false,终止时间，右下边界点的s,右上边界点的s,obs_id]
  ...
  //upper点的时间小于等于t表明该障碍物在t时刻之前对车无影响了
  for (const auto& obs_t_edge : new_t_edges) {//遍历这些相关的st
    if (std::get<0>(obs_t_edge) == 0) {//upper点
  ...
  //obs_id_to_decision_表示之前处理过的障碍物
  std::vector<std::string> obs_id_to_remove;
  for (const auto& obs_id_to_decision_pair : obs_id_to_decision_) {//遍历每个障碍物的decision
    auto obs_id = obs_id_to_decision_pair.first;
    auto obs_decision = obs_id_to_decision_pair.second;
    auto obs_st_boundary = obs_id_to_st_boundary_[obs_id];
    //如果该障碍物是超车决策，但是该障碍物的起始点小于当前时刻前0.5s,且结束时间小于当前时刻0.5s，
    //即在当前时刻前发生的，因此该决策不在需要考虑
    if (obs_decision.has_overtake() &&
        obs_st_boundary.min_t() <= t - kOvertakenObsCautionTime &&//0.5
  ...
  //根据之前处理过的障碍物的决策信息得到该时刻t的最小s与最大s
  //遍历需要考虑的障碍物决策，若是要超车则将最小s值设为障碍物的最大s值
  //若是停车或者让行，则最大s值设为障碍物的最小s值
  double s_min = 0.0;
  double s_max = planning_distance_;
  for (auto it : obs_id_to_decision_) {
    auto obs_id = it.first;
    ...
    double obs_s_min = 0.0;
    double obs_s_max = 0.0;
    obs_st_boundary.GetBoundarySRange(t, &obs_s_max, &obs_s_min);//得到上下边界
    if (obs_decision.has_yield() || obs_decision.has_stop()) {
...
bool STObstaclesProcessor::ComputeObstacleSTBoundary(
  ...
  *is_caution_obstacle = false;
  ...
  if (obs_trajectory.trajectory_point().empty()) {//障碍物没有轨迹，则说明其是一条空轨迹或者是静态障碍物
    ...
    const Box2d& obs_box = obstacle.PerceptionBoundingBox();//静态障碍物的box
    std::pair<double, double> overlapping_s;
    //overlapping_s保存的是该path与障碍相交的起始的点s,与最后相交的点s
    //first为第一个重叠点的s值，second为最后一个重叠点的s值
    ...
    *obs_caution_end_t = planning_time_;//静态障碍物的结束时间即规划周期时间
  }//静态障碍物处理结束 
  else {
...
int STObstaclesProcessor::GetSBoundingPathPointIndex(
  ...
  if (is_before) {
    int mid_idx = (start_idx + end_idx - 1) / 2 + 1;//中间点
...
bool STObstaclesProcessor::IsPathPointAwayFromObstacle(
    const PathPoint& path_point, const PathPoint& direction_point,//next的点
    const Box2d& obs_box, const double s_thresh, const bool is_before) {//s_thresh==0
  ...
  for (const auto& corner_pt : corner_points) {//得到障碍物的四个角点
...
bool STObstaclesProcessor::IsADCOverlappingWithObstacle(
  ...
  return obs_box.HasOverlap(adc_box);//(detail)
}
