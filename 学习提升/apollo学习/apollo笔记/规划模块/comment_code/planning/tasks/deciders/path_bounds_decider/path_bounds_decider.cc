...
Status PathBoundsDecider::Process(
  ...
  if (FLAGS_enable_skip_path_tasks && reference_line_info->path_reusable()) {//不成立(false,false)
    return Status::OK();
  }
  ...
  // Initialization.
  //计算初始规划点的adc_s,adc_l值,并计算初始规划点处的lane_width
  InitPathBoundsDecider(*frame, *reference_line_info);

  // Generate the fallback path boundary.应急路径边界
  ...
  //将fallback的bound的初始s值，以及s的分辨率与fallback_path_bound_pair的bound放入candidate_path_boundaries中
//并将其label设为fallback
  // If pull-over is requested, generate pull-over path boundary.（靠边停车）
...
void PathBoundsDecider::InitPathBoundsDecider(
  ...
  if (FLAGS_use_front_axe_center_in_path_planning) {//false
  ...
  adc_frenet_s_ = adc_sl_info.first[0];//s值
  adc_frenet_l_ = adc_sl_info.second[0];//l值
  adc_frenet_sd_ = adc_sl_info.first[1];
  adc_frenet_ld_ = adc_sl_info.second[1] * adc_frenet_sd_;
  double offset_to_map = 0.0;
  reference_line.GetOffsetToMap(adc_frenet_s_, &offset_to_map);//offset_to_map为adc_frenet_s_附近引导线上的点的l值
  adc_l_to_lane_center_ = adc_frenet_l_ + offset_to_map;//规划点到达引导线的l距离

  // ADC's lane width.得到路宽
  double lane_left_width = 0.0;
  double lane_right_width = 0.0;
  if (!reference_line.GetLaneWidth(adc_frenet_s_, &lane_left_width,
                                   &lane_right_width)) {//得到在s处车道的边界
...
Status PathBoundsDecider::GenerateFallbackPathBound(
  ...
  // 1. Initialize the path boundaries to be an indefinitely large area.
  //根据引导线s值(分辨率为0.5，初始值为adc_s,长度为100m)将path_bound初始化，path_bound[s,(-OO,+OO)]
  if (!InitPathBoundary(reference_line_info, path_bound)) {
...
bool PathBoundsDecider::FindDestinationPullOverS(
  ...
  const auto& routing_end = *(routing->routing_request().waypoint().rbegin());//导航目标点
  reference_line.XYToSL(routing_end.pose(), &destination_sl);//投影值
  const double destination_s = destination_sl.s();
  const double adc_end_s = reference_line_info.AdcSlBoundary().end_s();//车头位置
...
bool PathBoundsDecider::SearchPullOverPosition(
  ...
  if (pull_over_status.pull_over_type() ==
    ...
    if (!FindDestinationPullOverS(frame, reference_line_info, path_bound,//得到目标点的s值
                                  &pull_over_s)) {
  ...
  while ((search_backward && idx >= 0 &&
    ...
    if (is_feasible_window) {
      ...
      const double pull_over_l =
          std::get<1>(pull_over_point) + pull_over_space_width / 2.0 + FLAGS_pull_over_offset;//0，l的范围是boundary往里0.125的车宽
...
bool PathBoundsDecider::InitPathBoundary(
  ...
  for (double curr_s = adc_frenet_s_;
       curr_s < std::fmin(adc_frenet_s_ +
                          ...
                          reference_line.Length());//向前100m或者速度乘以8s的最大值
...
bool PathBoundsDecider::GetBoundaryFromRoads(
  ...
  for (size_t i = 0; i < path_bound->size(); ++i) {
    ...
    // 2. Update into path_bound.(左边减去车宽，右边+车宽)
...
bool PathBoundsDecider::GetBoundaryFromLanesAndADC(
    const ReferenceLineInfo& reference_line_info,
    const LaneBorrowInfo& lane_borrow_info, double ADC_buffer,//0.5，lane_borrow_info==no_borrow
    PathBound* const path_bound, std::string* const borrow_lane_type,
    bool is_fallback_lanechange) {//is_fallback_lanechange==true
  // Sanity checks.
  CHECK_NOTNULL(path_bound);
  ACHECK(!path_bound->empty());
  const ReferenceLine& reference_line = reference_line_info.reference_line();
  ...
  // ADC's position.
  double past_lane_left_width = adc_lane_width_ / 2.0;//规划点处lane的宽度/2
  double past_lane_right_width = adc_lane_width_ / 2.0;
  int path_blocked_idx = -1;
  bool borrowing_reverse_lane = false;
  //path_bound在初始化时，根据参考线的s值将其设为一个数组：path_bound[s,(0,+OO)]（s,lmin,lmax）
  for (size_t i = 0; i < path_bound->size(); ++i) {
    double curr_s = std::get<0>((*path_bound)[i]);
    // 1. Get the current lane width at current point.
    double curr_lane_left_width = 0.0;
    ...
    if (!reference_line.GetLaneWidth(curr_s, &curr_lane_left_width,
      ...
      double curr_road_right_width = 0.0;
      if (reference_line.GetRoadWidth(curr_s, &curr_road_left_width,
                                      &curr_road_right_width)) {//得到road的边界
        is_left_lane_boundary =
            (std::abs(curr_road_left_width - curr_lane_left_width) >
             boundary_buffer);//当road的边界与lane的边界差值大于0.05时说明是lane边界，不是road边界
        is_right_lane_boundary =
            (std::abs(curr_road_right_width - curr_lane_right_width) >
             boundary_buffer);
      }
      //该点在引导线上对应点的l值（即距离lane中心线的l值）
      reference_line.GetOffsetToMap(curr_s, &offset_to_lane_center);//加上补偿
      curr_lane_left_width += offset_to_lane_center;
      curr_lane_right_width -= offset_to_lane_center;
      past_lane_left_width = curr_lane_left_width;
      past_lane_right_width = curr_lane_right_width;
    }//s处的左右边界计算结束，curr_lane_left_width(+)，curr_lane_right_width(+)
    ...
    static constexpr double kMaxLateralAccelerations = 1.5;
    double offset_to_map = 0.0;
    reference_line.GetOffsetToMap(curr_s, &offset_to_map);//curr_s，path_bound中的一个s值，仍处于遍历该值的过程

//不清楚计算原理（adc_frenet_ld_计算原理不清楚）
...
//应急车道boundary计算
    if (config_.path_bounds_decider_config()
            .is_extend_lane_bounds_to_include_adc() ||//false
        is_fallback_lanechange) {//is_fallback_lanechange==true
      // extend path bounds to include ADC in fallback or change lane path
      // bounds.//根据车型计算

      //adc_l_to_lane_center_=规划点到达中心线的l距离
      //计算规划点处的bound
      double curr_left_bound_adc =
          std::fmax(adc_l_to_lane_center_,
    ...
    //正常车道boundary计算
    else {//对之前加上的车当前位置的L，减去，实现boundary为lane的真实bound
    ...
    // 4. Update the boundary.更新包含车身的bound（直接更新相应的path_bound）
    if (!UpdatePathBoundaryWithBuffer(i, curr_left_bound, curr_right_bound,
  ...
  TrimPathBounds(path_blocked_idx, path_bound);//修剪,将path_blocked_idx之后的bound删掉
...
std::vector<ObstacleEdge> PathBoundsDecider::SortObstaclesForSweepLine(
  ...
  for (const auto* obstacle : indexed_obstacles.Items()) {
    ...
    sorted_obstacles.emplace_back(
        1, obstacle_sl.start_s() - FLAGS_obstacle_lon_start_buffer,//0.4
...
bool PathBoundsDecider::UpdatePathBoundaryWithBuffer(
    ...
    PathBound* const path_boundaries, bool is_left_lane_bound,//默认两者都为false
  ...
  double left_adc_buffer_coeff =//1.0
      (is_left_lane_bound
           ? config_.path_bounds_decider_config().adc_buffer_coeff()
           : default_adc_buffer_coeff);
  double right_adc_buffer_coeff =//1.0
      (is_right_lane_bound
           ? config_.path_bounds_decider_config().adc_buffer_coeff()
           : default_adc_buffer_coeff);

  // Update the right bound (l_min)://即根据车的宽度更新，车底盘中线的左右边界（有符号）
  double new_l_min =
