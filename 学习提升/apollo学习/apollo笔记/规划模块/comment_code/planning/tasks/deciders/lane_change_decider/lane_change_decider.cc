...
//config
  ...
  prev_status->set_is_clear_to_change_lane(false);//设为false
//如果当前引导线是变道引导线（根据车所在的passage判断，在生成引导线时赋值），则需要变道
...
//初始状态均为CHANGE_LANE_FINISHED
  if (!prev_status->has_status()) {//若是之前状态为空，则将其设为CHANGE_LANE_FINISHED//作用先搁置
  ...
  bool has_change_lane = reference_line_info->size() > 1;//若是引导线个数大于1，则说明需要变道
  ADEBUG << "has_change_lane: " << has_change_lane;
  if (!has_change_lane) {//若只有一条引导线，则不会变道，更新状态返回即可
...
bool LaneChangeDecider::IsClearToChangeLane(
  ...
  double ego_start_s = reference_line_info->AdcSlBoundary().start_s();//车尾
  double ego_end_s = reference_line_info->AdcSlBoundary().end_s();//起始点出的车头
  double ego_v =
