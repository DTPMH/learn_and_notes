...
Status PathLaneBorrowDecider::Process(
  ...
  if (FLAGS_enable_skip_path_tasks && reference_line_info->path_reusable()) {//不成立(fasle,false)
    // for debug
...
bool PathLaneBorrowDecider::IsNecessaryToBorrowLane(
  ...
  if (mutable_path_decider_status->is_in_path_lane_borrow_scenario()) {//如果之前就是借的lane，
    ...
    if (!IsSidePassableObstacle(reference_line_info)) {//是否为不移动的障碍物
      return false;
    }
