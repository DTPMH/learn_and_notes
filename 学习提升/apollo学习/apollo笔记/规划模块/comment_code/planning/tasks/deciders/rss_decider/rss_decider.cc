...
Status RssDecider::Process(Frame *frame,
  ...
  double front_obstacle_distance = FLAGS_rss_max_front_obstacle_distance;//3000
  for (const auto *obstacle : path_decision->obstacles().Items()) {
    if (obstacle->IsVirtual()) {
      continue;
    }
    bool is_on_road = reference_line_info->reference_line().HasOverlap(//检测障碍物是否onroad,
        obstacle->PerceptionBoundingBox());
    if (!is_on_road) {//不是跳过，是则继续执行
      continue;
    }
    const auto &obstacle_sl = obstacle->PerceptionSLBoundary();//检测是否会和车辆碰撞，否则跳过
    if (obstacle_sl.end_s() <= ego_v_s_start) {//障碍物在车后面跳过
      continue;
    }
    //找到在车前与车最近的障碍物，并将其速度与sl_boundary赋值
    double distance = obstacle_sl.start_s() - ego_v_s_end;
    if (distance > 0 && distance < front_obstacle_distance) {//3000
