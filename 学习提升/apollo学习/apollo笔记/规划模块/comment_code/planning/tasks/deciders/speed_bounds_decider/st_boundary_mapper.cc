...
bool STBoundaryMapper::MapStopDecision(
  ...
  const double stop_ref_s =
      stop_sl_point.s() - vehicle_param_.front_edge_to_center();//停车点的s值
//以下是获得停车点的s值
  ...
  point_pairs.emplace_back(
      ...
      STPoint(s_max + speed_bounds_config_.boundary_buffer(),//0.25//0.1
              planning_max_time_));
...
bool STBoundaryMapper::GetOverlapBoundaryPoints(
  ...
  if (trajectory.trajectory_point().empty()) {//静态障碍物
    ...
    for (const auto& curr_point_on_path : path_points) {
      if (curr_point_on_path.s() > planning_max_distance_) {//当前s超出了最大的规划距离（path_data长度）
        break;
      }
    ...
    if (path_points.size() > 2 * default_num_point) {//进行path下采样
