...
void PiecewiseJerkSpeedProblem::CalculateKernel(std::vector<c_float>* P_data,
 ...
 // x(i)''^2 * (w_ddx + 2 * w_dddx / delta_s^2)
  ...
  for (int i = 0; i < kNumParam; ++i) {
    P_indptr->push_back(ind_p);//每列几个有效值
    for (const auto& row_data_pair : columns[i]) {
      P_data->push_back(row_data_pair.second * 2.0);//该数值
      P_indices->push_back(row_data_pair.first);//该数值处于第几行
      ++ind_p;
    }
  }
  P_indptr->push_back(ind_p);//每列之前共有几个有效值
}
