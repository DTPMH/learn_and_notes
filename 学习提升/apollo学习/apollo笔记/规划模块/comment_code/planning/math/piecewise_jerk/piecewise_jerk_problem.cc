...
OSQPData* PiecewiseJerkProblem::FormulateProblem() {
  // calculate kernel
  std::vector<c_float> P_data;//那几个有效数
  std::vector<c_int> P_indices;//对应的行数
  std::vector<c_int> P_indptr;//列数之前的有效数的数量
  CalculateKernel(&P_data, &P_indices, &P_indptr);//计算P阵
  ...
  CalculateAffineConstraint(&A_data, &A_indices, &A_indptr, &lower_bounds,
                            &upper_bounds);//计算A阵，l，u

  // calculate offset
  std::vector<c_float> q;
  CalculateOffset(&q);//引导线权重补偿
