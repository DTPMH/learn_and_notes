...
PiecewiseJerkPathProblem::PiecewiseJerkPathProblem(
    ...
    //构造函数：初始化x_bounds,dx_bounds,ddx_bounds,以及weight_x_ref_vec_

void PiecewiseJerkPathProblem::CalculateKernel(std::vector<c_float>* P_data,
                                               std::vector<c_int>* P_indices,
                                               std::vector<c_int>* P_indptr) {
  const int n = static_cast<int>(num_of_knots_);//一共多少个节点
  const int num_of_variables = 3 * n;//x.dx,ddx
  ...
  for (int i = 0; i < n - 1; ++i) {//n个节点
    columns[i].emplace_back(i, (weight_x_ + weight_x_ref_vec_[i]) /
                                   (scale_factor_[0] * scale_factor_[0]));//1.0
    ++value_index;
  }//n个节点遍历结束
...
void PiecewiseJerkPathProblem::CalculateOffset(std::vector<c_float>* q) {
  ...
  if (has_x_ref_) {
    for (int i = 0; i < n; ++i) {
      q->at(i) += -2.0 * weight_x_ref_vec_.at(i) * x_ref_[i] / scale_factor_[0];//1.0
    }
  }
