...
void Scenario::Init() {//
  ...
  for (const auto& stage_config : config_.stage_config()) {//位置在lane_follow_config.pb.txt
  ...
  current_stage_ =
      ...
      //根据场景创建相应的stage，lane_follow只有一个stage,lane_follow_stage
      //并调用stage.cc中的构造函数,创建相应的task
}

Scenario::ScenarioStatus Scenario::Process(
    const common::TrajectoryPoint& planning_init_point, Frame* frame) {
  //current_stage_==lane_follow_stage
  if (current_stage_ == nullptr) {//如果当前步骤为空，则返回未知状态
    AWARN << "Current stage is a null pointer.";
    return STATUS_UNKNOWN;
  }
  if (current_stage_->stage_type() == ScenarioConfig::NO_STAGE) {//若当前步骤为没有步骤，则返回完成状态
    scenario_status_ = STATUS_DONE;
    return scenario_status_;
  }
  auto ret = current_stage_->Process(planning_init_point, frame);//轨迹生成，放入frame->reference_line_info->Trajectory
  //lane_follow只会返回RUNNING与ERROR
  switch (ret) {
    case Stage::ERROR: {
