...
PullOverStatus CheckADCPullOver(
  ...
  const auto& pull_over_status =//位置在path_boundary中计算
  ...
  if (!pull_over_status.has_position() ||
    ...
    return UNKNOWN;
  }//如果pull_over没有停车位置则返回未知状态

  const auto& reference_line = reference_line_info.reference_line();
  common::SLPoint pull_over_sl;
  reference_line.XYToSL(pull_over_status.position(), &pull_over_sl);//停车位置
  ...
  if (distance >= scenario_config.pass_destination_threshold()) {//15
  ...
  if (distance <= -kStartParkCheckRange) {//如果距离大于3m，则说明仍在接近中
  ...
  const bool position_check = CheckPullOverPositionBySL(
      ...
      pull_over_status.theta(), true);//当s大于0小于0.5，且L差值小于1，且theta差值小于0.2时为true
...
bool CheckPullOverPositionBySL(const ReferenceLineInfo& reference_line_info,
  ...
  bool ret = (l_diff <= scenario_config.max_l_error_to_end_point() &&//1.0
              theta_diff <= scenario_config.max_theta_error_to_end_point());//0.2
  if (check_s) {
    ret = (ret && s_diff >= 0 &&
           s_diff <= scenario_config.max_s_error_to_end_point());//0.5
  }
