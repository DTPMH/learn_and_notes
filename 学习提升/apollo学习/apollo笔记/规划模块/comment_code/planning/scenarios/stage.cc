...
Stage::Stage(const ScenarioConfig::StageConfig& config,
  ...
  next_stage_ = config_.stage_type();//no
  ...
  for (int i = 0; i < config_.task_type_size(); ++i) {
    ...
    ACHECK(config_map.find(task_type) != config_map.end())//没找到则输出以下信息
    ...
    if (iter == tasks_.end()) {
      ...
      task_list_.push_back(iter->second.get());//创建任务列表
    }
  }
}
...
bool Stage::ExecuteTaskOnReferenceLine(
  ...
  for (auto& reference_line_info : *frame->mutable_reference_line_info()) {//遍历每条引导线
    if (!reference_line_info.IsDrivable()) {
      AERROR << "The generated path is not drivable";
      return false;
    }

    for (auto* task : task_list_) {//除去change_lane与borrow_lane的decider其余与on_lane_planning相同
    ...
    if (reference_line_info.speed_data().empty()) {//没有生成速度轨迹
