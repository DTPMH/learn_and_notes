...
Stage::StageStatus PullOverStageApproach::Process(
  ...
  bool plan_ok = ExecuteTaskOnReferenceLine(planning_init_point, frame);//根据引导线生成轨迹
  if (!plan_ok) {
    AERROR << "PullOverStageApproach planning error";
  }

  const auto& reference_line_info = frame->reference_line_info().front();//第一条引导线
  scenario::util::PullOverStatus status = scenario::util::CheckADCPullOver(
      ...
      injector_->planning_context());//检查车目前的pull_over状态(unknown.approaching,pass_destination,complete,fail)
  ...
  if (!candidate_path_data.empty()) {
    for (const auto& path_data : candidate_path_data) {
      if (path_data.path_label().find("pullover") == std::string::npos) {//没生成pull_over的路径，则退出for循环
        break;
      }
      ...
      for (size_t i = path_data.discretized_path().size(); i >= 1; --i) {
        if (path_data.frenet_frame_path().back().s() -
            ...
            kNumExtraTailBoundPoint * kPathBoundsDeciderResolution) {//20*0.5==10
          continue;
        }
