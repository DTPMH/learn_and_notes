...
bool ControlComponent::Init() {
  ...
  ACHECK(
      cyber::common::GetProtoFromFile(FLAGS_control_conf_file, &control_conf_))//control_conf.pb.txt
  ...
  if (!FLAGS_use_control_submodules &&//false
  ...
  chassis_reader_config.pending_queue_size = FLAGS_chassis_pending_queue_size;//10
  ...
  localization_reader_config.channel_name = FLAGS_localization_topic;///apollo/localization/pose
...
Status ControlComponent::ProduceControlCommand(
  ...
  // check estop
  estop_ = control_conf_.enable_persistent_estop()//false
  ...
  if (FLAGS_enable_gear_drive_negative_speed_protection) {//false
  ...
  if (estop_) {
    ...
    // 目前线控只使用了加速度信号，停车时加速度设为负
    control_command->set_acceleration(
        control_conf_.lon_controller_conf().standstill_acceleration());
  }
  // check signal
...
bool ControlComponent::Proc() {
  ...
  if (control_conf_.is_control_test_mode() &&//false
