...
bool LatController::LoadControlConf(const ControlConf *control_conf) {
  ...
  ts_ = control_conf->lat_controller_conf().ts();//0.01
  if (ts_ <= 0.0) {
    AERROR << "[MPCController] Invalid control update interval.";
    return false;
  }
  cf_ = control_conf->lat_controller_conf().cf();//155494.663
  cr_ = control_conf->lat_controller_conf().cr();//155494.663
  preview_window_ = control_conf->lat_controller_conf().preview_window();//0
  lookahead_station_low_speed_ =
      control_conf->lat_controller_conf().lookahead_station();//1.4224
  lookback_station_low_speed_ =
      control_conf->lat_controller_conf().lookback_station();//2.8448
  lookahead_station_high_speed_ =
      control_conf->lat_controller_conf().lookahead_station_high_speed();//1.4224
  lookback_station_high_speed_ =
      control_conf->lat_controller_conf().lookback_station_high_speed();//2.8448
  wheelbase_ = vehicle_param_.wheel_base();//2.8448
  steer_ratio_ = vehicle_param_.steer_ratio();//6.7142857
  steer_single_direction_max_degree_ =
      vehicle_param_.max_steer_angle() / M_PI * 180;//8.20304748437
  max_lat_acc_ = control_conf->lat_controller_conf().max_lateral_acceleration();//5
  low_speed_bound_ = control_conf_->lon_controller_conf().switch_speed();//2.0
  low_speed_window_ =
      control_conf_->lon_controller_conf().switch_speed_window();//1.0

  const double mass_fl = control_conf->lat_controller_conf().mass_fl();//530
  const double mass_fr = control_conf->lat_controller_conf().mass_fr();//530
  const double mass_rl = control_conf->lat_controller_conf().mass_rl();//530
  const double mass_rr = control_conf->lat_controller_conf().mass_rr();//530
  const double mass_front = mass_fl + mass_fr;
  const double mass_rear = mass_rl + mass_rr;
  mass_ = mass_front + mass_rear;//1060

  lf_ = wheelbase_ * (1.0 - mass_front / mass_);//0.0
  lr_ = wheelbase_ * (1.0 - mass_rear / mass_);//0.0

  // moment of inertia
  iz_ = lf_ * lf_ * mass_front + lr_ * lr_ * mass_rear;//0

  lqr_eps_ = control_conf->lat_controller_conf().eps();//0.01
  lqr_max_iteration_ = control_conf->lat_controller_conf().max_iteration();//150

  query_relative_time_ = control_conf->query_relative_time();//0.8

  minimum_speed_protection_ = control_conf->minimum_speed_protection();//0.1
...
Status LatController::ComputeControlCommand(
  ...
  trajectory_analyzer_ =
      std::move(TrajectoryAnalyzer(&target_tracking_trajectory));//将轨迹点与head赋值trajectory_analyzer_
  ...
  if (((FLAGS_trajectory_transform_to_com_reverse &&//true
        vehicle_state->gear() == canbus::Chassis::GEAR_REVERSE) ||
       (FLAGS_trajectory_transform_to_com_drive &&//true
...
void LatController::ComputeLateralErrors(
  ...
  // FOR TEST
  // double s_error = cos_target_heading * dx + sin_target_heading * dy;
  // debug->set_ref_heading(
  //   target_point.path_point().theta() + target_point.path_point().kappa() * s_error);
