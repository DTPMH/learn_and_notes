...
Status LonController::Init(std::shared_ptr<DependencyInjector> injector,
  ...
  dirve_by_ThrottlenBreak_ = !(lon_controller_conf.drive_by_acceleration());
  ...
  // Decide wether to use throttle & break
  if (dirve_by_ThrottlenBreak_) {
...
Status LonController::ComputeControlCommand(
  ...
  // Pass while not using Throttle & break
  if (dirve_by_ThrottlenBreak_ && !control_interpolation_) {
  ...
  if (dirve_by_ThrottlenBreak_) {
    ...
    /*
    // 保证加速阶段油门开度必须大于零，减速时刹车必须大于零，但实际减速时不一定要踩刹车
    ...
    */
    if (calibration_value >= 0) {
      throttle_cmd = calibration_value;
      brake_cmd = 0.0;
    } else {
      throttle_cmd = 0.0;
      brake_cmd = calibration_value;
    }
  ...
  if (FLAGS_enable_csv_debug && speed_log_file_ != nullptr) {
    fprintf(speed_log_file_,
            ...
            debug->speed_lookup(), debug->calibration_value(), throttle_cmd,
            brake_cmd, debug->is_full_stop());
  }
